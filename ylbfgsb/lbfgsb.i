...
/* PROTOTYPE
     int lbfgsb(int n, int m, double array x, double array f,
                double array g, pointer l, pointer u,
...
/* DOCUMENT ws = lbfgsb_setup(m, x);
   ...
   KEYWORDS
     INF     can be used to specify the lower bounds on X.  INF must have as
           many elements as X.

     SUP     can be used to specify the upper bounds on X.  SUP must have as
           many elements as X.

     BND   can be used to specify the  type of bounds imposed on the
     ...
     FACTR can be used to tune the stopping criterion based on the
           convergence of the penalty function F.  The iteration will
           stop when

             (F^k - F^{k+1})/max{|F^k|,|F^{k+1}|,1} <= FACTR*EPSMCH

           where EPSMCH is the machine precision, which is automatically
           generated by the code. Typical values for FACTR: 1e+12 for
           low accuracy; 1e+7 for moderate accuracy; 1e+1 for extremely
           high accuracy.  The default is FACTR=1e7.

     PGTOL can be used to tune the stopping criterion based on the projected
           gradient.  The iteration  will stop when
 ...
 */
{
  ...
  isave = array(int, 3*n + 27);

  ws = [&int(m), &int(n), &inf, &sup, &bnd, &factr, &pgtol, &ctask, &csave,
        &isave, &dsave];
  job = __lbfgsb__(n, m, x, 0.0, x, ws(3), ws(4), *ws(5), *ws(6),
                   *ws(7), *ws(8), *ws(9), *ws(10), *ws(11), -1);
  if (job != LBFGSB_FG) error, lbfgsb_message(ws);
  return ws;
}

func lbfgsb_message(ws, csave) { return string(ws((csave ? 9 : 8))); }
...
func lbfgsb_iterate(&x, &f, &g, ws)
/* DOCUMENT job = lbfgsb_iterate(x, f, g, ws);

     Perform next LBFGSB step.  Argument X is for the current parameters, F
     and G are the function value and the gradient at X,  WS is the
     workspace as returned by lbfgsb_setup (which see).  The returned value
     is one of:
 ...
 */
{
  ...
  return __lbfgsb__(n, m, x, f, g, ws(3), ws(4), *ws(5), *ws(6),
                    *ws(7), *ws(8), *ws(9), *ws(10), *ws(11), -1);
}
LBFGSB_START       = 0n;
LBFGSB_FG          = 1n;
